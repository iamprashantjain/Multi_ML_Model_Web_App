...
user_menu = st.sidebar.selectbox('Select Options',('CV (CS)','CV (NCS)','2W (CS)','2W (NCS)'))
...
if user_menu == "CV (CS)":
	...
	METER_READING_cleaned = col7.number_input('Enter Meter Reading',min_value=None)
	...
	cust_seg = df['Customer_Segmentation'].loc[df['Make_Clean'] == Make_Clean].drop_duplicates().tolist()
	cust_seg.insert(0,'Select Segmentation')
	Customer_Segmentation = col6.selectbox("Select Customer Segmentation",cust_seg,index = 0)
	...
	X = pd.DataFrame(columns=['MAKE_YEAR','Make_Clean','Model_Clean','Variant_Clean','Fuel_Clean','CV_State_Clean','METER_READING_cleaned'],data=np.array([MAKE_YEAR,Make_Clean,Model_Clean,Variant_Clean,Fuel_Clean,CV_State_Clean,METER_READING_cleaned]).reshape(1,7))
